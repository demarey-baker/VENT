...
if(isset($_POST) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest'){
	...
	$results = $mysqli->query("SELECT COUNT(*) FROM event WHERE EventType='$q'");
	...
	$results = $mysqli->prepare("SELECT EventID,EventName,EventLocation,DAYNAME(StartDate) as day, MONTHNAME(StartDate) as month,DAY(StartDate) as daynum, StartTime,EventCost, EventImage FROM event WHERE EventType='$q' LIMIT $page_position, $item_per_page");
	...
	while($results->fetch()){ //fetch values
		...
		echo "<div class=\"row\">
						...
						echo "<label title=\"Click to see the guestlist\" class=\"btn btn-link btn fsharebtns floatRight\">
					...
					{
						echo "<p class=\"costLbl\">FREE <a href=\"\" class=\" floatRight\">Reviews/Comments</a></p>";
					}
					else 
					{
						echo "<p class=\"costLbl\">&#36;{$EventCost} <a href=\"\" class=\" floatRight\">Reviews/Comments</a></p>";
					}
